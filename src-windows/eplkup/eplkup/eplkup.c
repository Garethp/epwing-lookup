...
/* For Rikaisama, comment this out to ensure that console window does not
   popup when eplkup is called. */
...
static int dump_text_enabled           = 0;        /* 1 = Perform text dump instead of normal word lookup */
static int dump_text_page              = 0;        /* Page to use when dumping text */
static int dump_text_offset            = 0;        /* Offset to use when dumping text */
...
int main(int argc, char *argv[])
{
  ...
  /* If the user wants to perform a text dump instead of performing a search */
  if(dump_text_enabled)
  {
    dump_text();
  }
  /* Else If the user wants to print the subbook title to the output file instead of performing a search */
  else if(print_title)
  {
...
--
-- Description:
--   Dump all text in dictionary to file starting at the specified
--   page and offset.
--
-- Parameters:
--   None.
...
static void dump_text(void)
{
  ...
  int i = 0;
  unsigned long text_count = ULONG_MAX; /* Arbitrarily high in order to make sure that we reach the end */
  ...
  /* Note: To get the page and offset info, in EBWIN goto "View | TagInfo" and look
           at the title text of the dialog that pops up.
     Example: For Ken5, The first entry "ああ" is at page 108260, offset 1610. */

  text_position.page = dump_text_page;
  text_position.offset = dump_text_offset; 

  if(error_code != EB_SUCCESS)
  {
    fprintf(stderr, "Error: Failed to get text information, \"%s\"\n", eb_error_message(error_code));
    die(1);
  }
  ...
  while(i < text_count)
  {
    ...
    {
      fprintf(stderr, "Error: eb_forward_text, \"%s\"\n", eb_error_message(error_code));
      fclose(out_file);
      die(1);
    }
...
void parse_command_line(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++)
  {
    if(strcmp(argv[i], "--dump-text") == 0)
    {
      dump_text_enabled = 1;

      i++;
      dump_text_page = atoi(argv[i]);

      i++;
      dump_text_offset = atoi(argv[i]);

      if(dump_text_page < 0)
      {
        usage(1, "Dump text page must be >= 0.\n");
      }

      if(dump_text_offset < 0)
      {
        usage(1, "Dump text offset must be >= 0.\n");
      }
    }
    else if(strcmp(argv[i], "--emphasis") == 0)
    {
      set_emphasis_hook = 1;
    }
    ...
    {
      usage(0, "eplkup version 1.6 by Christopher Brochtrup.\n");
    }
    else
    {
      if(dump_text_enabled)
...
void usage(int exit_code, char *msg)
{
  ...
  printf("Usage: eplkup [options] <book-path> <input-file> <output-file>\n");
  printf("       (Do not specify <input-file> if using the --dump-text option)\n");
  printf("\n");
  ...
  printf("\n");
  printf("Optional:\n");
  printf("  --dump-text <page> <offset> - Dump all text from dictionary to file starting at provided page and offset.\n");
  printf("  --emphasis   - Place HTML <em></em> tags around bold/emphasized text.\n");
  printf("  --gaiji <arg> - 0 = Replace gaiji with no UTF-8 equivalents with a '?' (default).\n");
  printf("                  1 = Replace gaiji with no UTF-8 equivalents with HTML image tags containing\n");
  printf("                      embedded base64 image data.\n");
  printf("  --help       - Show help.\n");
  printf("  --hit <hit>  - Specify which hit to output (starting at 0). If not specified, all hits will be output.\n");
  ...
  printf("  --max-hits <hits> - Specify the number of hits to output when --hit is not specified. Default is %d.\n", MAX_HITS);
  ...
  printf("  --subbook <subbook> - The subbook to use in the EPWING book. Default is 0.\n");
